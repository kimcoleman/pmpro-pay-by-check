...
function pmpropbc_checkout_boxes()
{
	...
	{
	?>
	<div id="pmpro_payment_method" class="pmpro_checkout" <?php if(!empty($pmpro_review)) { ?>style="display: none;"<?php } ?>>
		<hr />
		<h3>
			<span class="pmpro_checkout-h3-name"><?php _e('Choose Your Payment Method', 'pmpro-pay-by-check'); ?></span>
		</h3>
		<div class="pmpro_checkout-fields">
			<span class="gateway_<?php echo esc_attr( $gateway_setting ); ?>">
				<input type="radio" name="gateway" value="<?php echo esc_attr( $gateway_setting ); ?>" <?php if(!$gateway || $gateway == $setting_gateway) { ?>checked="checked"<?php } ?> />
			...
			<?php
				...
				<span class="gateway_paypalexpress">
					...
					<a href="javascript:void(0);" class="pmpro_radio"><?php _e('Check Out with PayPal', 'Check Out with PayPal', 'pmpro-pay-by-check' ); ?></a>
				</span>
				<?php
				}
			?>
